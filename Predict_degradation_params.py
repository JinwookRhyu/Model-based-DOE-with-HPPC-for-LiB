...
W_list = np.array([0.7328137506082335, 0.1868910501782918, 0.578849789166241, 0.1873575283040331, 0.4972712014733726, 0.19924379396233313, 0.4521380016118698, 0.21624052890683618, 0.42655534487697977, 0.23632746559849832])
...
deg_params_range = np.reshape(
    np.concatenate((R_f_c_range, c_tilde_c_range, R_f_a_range, c_tilde_a_range, c_lyte_range), axis=0), (1, 10))

deg_params_lower = np.array(
    [deg_params_range[0][0], deg_params_range[0][2], deg_params_range[0][4], deg_params_range[0][6],
     deg_params_range[0][8]])
deg_params_upper = np.array(
    [deg_params_range[0][1], deg_params_range[0][3], deg_params_range[0][5], deg_params_range[0][7],
     deg_params_range[0][9]])

# Directory for saving MCMC figures and autocorrelation figures
if is_balanced:
    dir_savefig = os.getcwd() + "/I_err_" + str(I_err) + "_" + mode + "_N" + str(
        N) + "_predict_balanced"
    dir_txtfile = os.getcwd() + "/error_bound_optimal_output_" + tpe + "_N" + str(N) + "_balanced"
else:
    dir_savefig = os.getcwd() + "/I_err_" + str(I_err) + "_" + mode + "_N" + str(
        N) + "_predict_unbalanced"
...
for mm in range(1):
    ...
    # Spread walkers
    initial = np.array([deg_params_range[0][0], deg_params_range[0][2], deg_params_range[0][4], deg_params_range[0][6],
                        deg_params_range[0][8]])
    ndim = len(initial)
    rng = np.random.RandomState(0)
    p0 = [np.array(initial) + np.multiply(np.array(
        [deg_params_range[0][1] - deg_params_range[0][0], deg_params_range[0][3] - deg_params_range[0][2],
         deg_params_range[0][5] - deg_params_range[0][4], deg_params_range[0][7] - deg_params_range[0][6],
         deg_params_range[0][9] - deg_params_range[0][8]]), rng.uniform(0, 1, ndim)) for i in
          range(nwalkers)]

    savename = dir_savefig + "/MCMC_opt_" + str(
        rxn_method) + "_" + str(tpe) + "_" + str(int(1000 * V_limit)) + "mV_" + str(N) + "N_predict.png"
    ...
    if __name__ == '__main__':
        ...
        fig = corner.corner(samples, show_titles=True, labels=labels, plot_datapoints=True,
                            ...
                            title_fmt='.3f', range=[(deg_params_range[0][0], deg_params_range[0][1]),
                                                    (deg_params_range[0][2], deg_params_range[0][3]),
                                                    (deg_params_range[0][4], deg_params_range[0][5]),
                                                    (deg_params_range[0][6], deg_params_range[0][7]),
                                                    (deg_params_range[0][8], deg_params_range[0][9])],
                            plot_contours=True)
        ...
        for k in range(N):
            fig_title = fig_title + '\n' + "#{0:<2} = ({1:.3f}, {2} mV, {3:.3f})".format(int(k + 1), c_c[k], round(
                delta_V[
                    k] * 1000), W_list[k])  # ' + str(k+1) + ' =\t(' + str(c_c[k]) + ',\t' + str(round(delta_V[k]*1000)) + ' mV)'
...
if is_balanced:
    with open(os.getcwd() + "/" + mode + "_N" + str(N) + "_predict_balanced.pkl", 'wb') as f:
        pickle.dump(results, f)
else:
    with open(os.getcwd() + "/" + mode + "_N" + str(N) + "_predict_unbalanced.pkl", 'wb') as f:
        pickle.dump(results, f)
